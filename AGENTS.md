...
- `/r/<base64url-of-target>?<options>&<page-query>` â ScramJet, the server-side rewriter. Any non-`lb_` query keys belong to the target page and are forwarded to it.
- `/lj/<base64url-of-target>?...` â LobsterJet entry. Same handler (`engine_proxy`). Pages entered via `/lj/` get their links and subresources rewritten to `/lj/` so the service worker can cache them; pages entered via `/r/` stay on `/r/`.

Engine option keys (all `lb_`-prefixed; legacy bare keys are still accepted for old saved routes):

- `lb_ab=1` â adblock filter stripping (built-in host list + `adblock-extra.txt`)
- `lb_trk=1` â tracker stripping (+ `tracker-extra.txt`)
- `lb_https=1` â reject plain-http targets
- `lb_img=1` â re-encode JPEGs at lower quality
- `lb_ua=<urlencoded UA>` â user-agent override
- `lb_hdrs=<base64url of "Name: value" lines>` â custom outbound headers (still honored server-side, but the UI option was removed by request and nothing sends it)
...
Link prefetch: the React app (not the shim â the app has same-origin access to the proxied frame documents) attaches pointerover/focusin listeners to each frame document and fetches hovered links' routes through the worker, warming the cache before the click. Wiring lives in the same-origin poll effect in `ui/src/pages/Browser.tsx`, guarded by a WeakSet of wired documents. It skips `target="_blank"` and download links.

The app registers the worker in `ui/src/App.tsx`. `routeUrl()` in `ui/src/settings.ts` builds `/lj/` routes when `settings.proxyEngine === "lobsterjet"` (the default); the shim inside proxied pages derives its own route prefix from `location.pathname`. Without the worker installed, everything still works â the server answers `/lj/` identically.

### Search suggestions

`GET /suggest?engine=<id>&q=<query>` â server-side proxy for open suggestion (osjson) APIs to dodge CORS:
...
- Tab hover preview: a fixed panel under the strip renders a live but scriptless iframe (`sandbox="allow-same-origin"`, no allow-scripts â the page's JS never runs twice and audio can never double) plus a mute toggle. Muting records the tab in `mutedTabs`; the same-origin poll tick re-asserts `muted = true` on every audio/video in that tab's frame because pages keep creating new media elements.
- There is NO hamburger anywhere (removed by request; do not re-add one, and no `m3e-nav-rail-toggle`). The nav rail is compact-only. Entering the browse view slides the rail off-screen with the rest of the chrome (`railHidden` in App.tsx adds `.lb-rail-hidden`: `margin-left: calc(-1 * compact-width + 6dp)`), leaving a 6dp sliver clickable; clicking the rail itself (sliver or full) toggles it. Clicks on `m3e-nav-item` children must NOT toggle it (App.tsx rail onClick ignores `e.target.closest("m3e-nav-item")`). Padding globally uses the M3E measurement tokens (`--md-sys-measurement-spaceN`, N = px * 12.5, e.g. space100 = 8dp); the rail top/bottom spaces are the component defaults. Never override the rail width/overflow.
- CRITICAL (React + m3e custom elements): React does NOT map `className` to the `class` attribute on custom elements (it emits a literal `classname` attribute, which no CSS matches). Never pass `className` or a bare `ref` prop to an `m3e-*` element. Manage classes imperatively: `const r = useRef<any>(null)` + `r.current?.classList.toggle(...)` in an effect, and attach it with a JSX spread `{...{ ref: r }}` (the m3e type defs reject a bare `ref` attribute).
...
- The bottom dock and tab strip share the `dockTucked` idle-tuck state; transforms must be applied to plain wrapper divs (`.lb-dock-pill`), not the m3e-toolbar host â Lit host display makes transforms silently do nothing.
...
Render service `srv-daq17cmk1f9s73djhta0` (workspace `tea-da6k16hsrm7s73aeg0s0`). The Dockerfile builds the server (`cargo build --release -p lobster-server`) then the UI (`npm ci && npm run build` in `ui/`), so a Rust or TS type error fails the deploy. Builds take roughly 2â10 minutes.
...
- Incognito mode: no history recording, no session persistence. The toggle is an icon button (inline domino-mask SVG â `incognito` glyph missing from the font). Toggling ON suspends the whole normal session (stashed in a ref in App.tsx, never persisted) and opens one fresh empty incognito tab; toggling OFF closes the incognito tabs and restores the suspended session. The session-swap logic lives in `toggleIncognito` in `ui/src/App.tsx`.
