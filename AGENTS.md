...
- There is NO hamburger anywhere (removed by request; do not re-add one, and no `m3e-nav-rail-toggle`). The nav rail is compact-only. Entering the browse view slides the rail off-screen with the rest of the chrome (`railHidden` in App.tsx adds `.lb-rail-hidden`: `margin-left: calc(-1 * compact-width + 6dp)`), leaving a 6dp sliver clickable; clicking the rail (sliver or full) toggles it. Padding globally uses the M3E measurement tokens (`--md-sys-measurement-spaceN`, N = px * 12.5, e.g. space100 = 8dp); the rail top/bottom spaces are the component defaults. Never override the rail width/overflow.
