...
- The nav rail is compact-only; no expand/toggle, no `m3e-nav-rail-toggle`. On Home/Settings the rail is simply there, no hamburger. The hamburger exists ONLY in the browse view: a floating `.lb-rail-menu` icon button rendered OUTSIDE the m3e-nav-rail element (so it stays put while the rail slides). Entering browse auto-hides the rail (`railHidden` in App.tsx adds `.lb-rail-hidden`, which slides the whole rail off-screen via `margin-left: -4.5rem`); the hamburger toggles it back and forth. It is centered on the rail axis with `left: 16px` (40px button in the 72px compact rail). Never override the rail width/overflow.
