...
- The nav rail is compact-only: there is no expand/toggle function and no `m3e-nav-rail-toggle`. While the browser view is active the rail collapses (`railHidden` state in App.tsx adds `.lb-rail-hidden`, hiding the nav items) to just the hamburger; the hamburger appears ONLY in that collapsed state and clicking it brings the rail back. It is centered on the rail axis by `#nav-rail > m3e-icon-button { margin-inline: auto !important; }` (the `--m3e-nav-rail-icon-button-inset` token is a left margin, not a centering inset). Rail geometry is trimmed via the `--m3e-nav-rail-*` tokens (4.5rem compact width, 0.5rem inline padding), never via width/overflow overrides.
